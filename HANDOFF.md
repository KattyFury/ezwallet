...
- **🔴🔴 08-04d/f – ĐÃ TẮT HẲN `setCustomSecurityQuestions()` (mọi tham số, kể cả chỉ truyền `securityConfirmItems` không kèm `questions`).** Đo thật 2 lần liên tiếp trên deploy: lần 1 truyền `{ questions: [8 câu VN], securityConfirmItems }` → màn Câu hỏi bảo mật RỖNG TOÀN BỘ (chặn tạo ví). Bỏ `questions`, chỉ còn `{ securityConfirmItems }` → **VẪN RỖNG Y HỆT**. Tức lỗi KHÔNG phải do nội dung `questions`, mà nghi ngờ **gọi method này (bất kể tham số gì) khiến SDK thay hẳn bộ câu hỏi mặc định bằng danh sách RỖNG** thay vì tự rơi về mặc định của Circle. Đã comment out CẢ 3 chỗ gọi (`circle.js` + `Login.jsx` ×2) — giờ hoàn toàn không gọi method này nữa, màn Câu hỏi bảo mật về lại mặc định English (chưa test lại nhưng đây là baseline trước khi đụng vào bug này). **Hệ quả:** 3 dòng cảnh báo màn Xác nhận bảo mật + bộ câu hỏi tiếng Việt (`CIRCLE_SECURITY_QUESTIONS`, đã soạn sẵn) ĐÀNH tiếng Anh cho tới khi debug được. **ĐỪNG gọi lại `setCustomSecurityQuestions()` dưới bất kỳ hình thức nào cho tới khi test được với `questions` ĐẦY ĐỦ + nhiều hơn (nghi ngờ có ngưỡng tối thiểu số câu, thử 12-15) và xác nhận màn không rỗng** — đây là màn GATE cả luồng tạo ví, vỡ là user mới không tạo được tài khoản.
