...
- **MÀN CHÀO "Thêm vào màn hình chính" (`AddToHome.jsx`, 08-12)** – hiện TRƯỚC cả Login/PinGate, **chỉ khi mở bằng trình duyệt trên điện thoại**. Đã thêm vào màn hình chính (app chạy standalone) → không hiện; máy tính → không hiện (không có màn hình chính để thêm). Bấm "Bỏ qua" ghi `ez_a2hs_skipped` → không hỏi lại. Bố cục: cụm chữ absolute **top 25dvh** (giữa hàng 3) viết xuống; **hàng 9 = nút "Bỏ qua" XANH** `.row10-single .btn-primary` (KHÔNG đỏ – đỏ trong app nghĩa là mất tiền/lỗi, mà bỏ qua thì không phá gì). iPhone 5 bước (Safari), Android 4 bước (Chrome) – **cả 2 đều hướng dẫn BẤM TAY** (user chốt 08-12). ⚠️ **CỐ Ý KHÔNG dùng nút cài 1 chạm `beforeinstallprompt`**: iPhone vốn không có nó (Apple không hỗ trợ) nên 2 hệ sẽ lệch nhau, và nó đòi app phải có **service worker** – app này chưa có. Muốn làm nút 1 chạm cho Android sau này thì phải thêm service worker + icon 192px vào manifest trước. Check standalone phải đủ **CẢ HAI** `display-mode: standalone` và `navigator.standalone` (iPhone <16.4 chỉ có cái sau). Đo Playwright 08-12 (390/375/412): cụm chữ đúng 25.0dvh, tâm nút đúng 90.0dvh, nút rộng đúng 75vw, không tràn ngang, chữ không đè nút.
