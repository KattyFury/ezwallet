...
- **DROP SHADOW nút gradient (user chốt 07-22c):** nút NỔI/bấm được nền gradient có bóng **THẲNG ĐỨNG** (offset-x 0, đổ dọc xuống — hợp hướng gradient dọc) `box-shadow: 0 4px 12px rgba(<màu nút>, .30)`: `.btn-primary` + `.action-card.primary` bóng brand (11,83,191), `.btn-success` xanh lá (22,163,74), `.btn-error` đỏ (220,38,38). "1 tí" thôi — đừng làm đậm. Nút `.btn-secondary`/action-card trắng KHÔNG bóng (đã có viền xám làm tín hiệu bấm).
