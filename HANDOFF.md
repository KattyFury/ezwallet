...
- **Circle SDK UI (VI) ✅ (2026-07-01, làm lại ĐÚNG):** `src/circle.js` `getSDK()` gọi `setLocalizations(PIN_VI)` — Việt hóa common + enterPincode (headline2:'' để hết "giao dịchPIN") + init/confirm/new PIN + recoverPincode + securityIntros/Questions/Summary + **securityConfirm với hướng dẫn ĐÚNG** (`inputHeadline='Gõ chính xác dòng chữ: I agree'`, `inputPlaceholder='I agree'` — user vẫn phải gõ "I agree" vì Circle hardcode, KHÔNG auto-điền được). **CẢNH BÁO:** chưa test tạo ví MỚI với securityConfirm VI này — nếu nút Tiếp tục không sáng ở bước "I agree", revert securityConfirm về English. Chữ LỖI runtime trong iframe vẫn English (không localize được — xem Gotchas).
