...
- **DROP SHADOW nút bấm được (user chốt 07-22d — bản cuối):** bóng **THẲNG ĐỨNG** (offset-x 0, đổ dọc) `box-shadow: 0 4px 6px ...` — **VỪA nhưng KHÔNG TỎA** (blur nhỏ 6px, đừng để 12px = diffuse/nhạt). Nút GRADIENT bóng tô đúng màu nút alpha .42: `.btn-primary` + `.action-card.primary` brand (11,83,191), `.btn-success` lá (22,163,74), `.btn-error` đỏ (220,38,38). Nút TRẮNG **CŨNG có bóng** (user đổi ý so với 07-22c): `.btn-secondary` + `.action-card` (thường) `rgba(0,0,0,.16)`. ĐỪNG tăng blur (thành tỏa) hay hạ alpha (thành nhạt).
