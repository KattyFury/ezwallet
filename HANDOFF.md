...
**Cập nhật:** 2026-07-29 · **Repo:** https://github.com/KattyFury/ezwallet · **Live:** https://ezwallet.pages.dev (Cloudflare Pages, auto-deploy từ `main`) · **Local:** `D:\Files\Claude\build_on_arc\ezwallet`
...
- **NÚT ĐỨNG ĐƠN ĐỘC = RỘNG 3/4 MÀN (user chốt 07-29 — "cho các button đứng đơn độc to ra cùng 1 dạng cho đồng bộ"):** `width: min(75vw, calc(var(--screen-max) * 0.75))` (neo vào .screen, KHÔNG dùng % vì khung cha đang thụt lề 20px → % ra số khác nhau mỗi màn). Đã áp: "Hold to show tokens" (HomeSend), "Tap to copy your wallet address" (HomeReceive), **nút Swap** (trước 66.67%). Đo Playwright: 3 nút = 293px @390 · 281px @375, BẰNG NHAU. Cặp nút `.row10-dual` (44% mỗi nút) giữ nguyên. ⚠️ `.row10-single .btn` vẫn 66.67% — CHƯA hỏi user có đổi 3/4 luôn không, đừng tự sửa.
