...
**Cập nhật:** 2026-07-04 (session 18 — batch UX: menu rows, bỏ onboarding, QR đặt tên, faucet copy, swap notif rõ, ErrorBoundary)
...
**❌ Google login — VẪN DISABLED (nút mờ Login.jsx `disabled: true`). User đi hỏi dev trước khi khui (2026-07-04).**
...
- **🔑 ĐÍNH CHÍNH HIỂU LẦM (S16, đọc docs Circle):** KHÔNG phải Circle "chặn PIN cho SSO". Sự thật: **social login + email OTP duyệt giao dịch bằng CONFIRMATION UI (xem trước + đồng ý), KHÔNG dùng PIN**; *"Confirmation UIs are NOT available for the PIN authentication method"* (docs). PIN là auth method RIÊNG. → 403 khi gọi `PUT /user/pin` cho user Google là vì **user SSO KHÔNG CÓ PIN** (gọi sai loại API), không phải bị chặn. App cũ ép PIN làm authorizer chung → mismatch.
