...
**Cập nhật:** 2026-07-06 (session 24 — NGHỈ BUILD. Language/Currency: hiện option Việt/Trung + CNY/VND nhưng KHOÁ (chỉ English + USD/EUR chọn được); Faucet → thông báo "Faucet successful". [S23: TxHistory "Add to Contacts" + mờ đáy list, Security bỏ Recovery] [S22: local dev-server, lề 20px, scrollbar gutter, nút vị trí 5/9, no line xám])
...
- **App KHOÁ English (S24, user chốt):** `i18n.js` `LANG = 'en'` cứng. Màn Language & Currency có HIỆN option Việt/Trung (ngôn ngữ) + CNY/VND (tiền tệ) cho popup đỡ trống nhưng **KHOÁ** (nút `disabled`, mờ, không bấm); chỉ English + USD/EUR chọn được. Lý do khoá: Circle SDK chỉ English + nhiều chuỗi mới hardcode English → VI/ZH sẽ lẫn Anh. Hạ tầng VI/ZH (`i18n` dicts) + `setLang()`/`detect()` còn nguyên — mở lại: cho `LANG` đọc localStorage + bỏ `locked` 2 option đó + (tiền tệ) thêm rate CNY/VND ở `chain.js getDisplayRates` + `SUPPORTED_CURRENCIES`.
...
- **S24 (07-06, NGHỈ BUILD):** **(1) Language/Currency KHOÁ (sửa lại S23):** ngôn ngữ giữ `LANG='en'` cứng — popup HIỆN English (chọn) + Tiếng Việt/中文 (KHOÁ, nút disabled mờ); tiền tệ popup HIỆN USD/EUR (chọn) + CNY/VND (KHOÁ). Revert hết plumbing CNY/VND (chain.js `fetchFiatRates`, data.js `SUPPORTED_CURRENCIES`/`CURRENCY_LABEL`/`displayNum`, fallback rate 4 file, BalanceHeader auto-shrink) vì không dùng — chỉ còn UI option khoá. `Picker` component dùng chung cho 2 popup. **(2) Faucet → thông báo riêng:** bấm nút Faucet (HomeSend) set `localStorage.ez_faucet_pending=now`; NotifArea `pollIncoming` thấy USDC vào ĐẦU TIÊN trong 1h + cờ set → thông báo **"Faucet successful · received X USDC"** (thay vì "nhận từ 0x…lạ"), rồi xoá cờ. → **Dừng build, sản phẩm ở trạng thái ổn định.**
- **S23 (07-06):** **(1) TxHistory:** nút `+ Add` → `Add to Contacts` (rõ nghĩa); thêm mask mờ ĐÁY list (`linear-gradient to top ... calc(100dvh/30)`, đồng bộ NotifArea) — token list HomeSend đã có sẵn. **(2) MỞ LẠI chọn ngôn ngữ:** `LANG` đọc `localStorage.ez_lang` (mặc định 'en'); Language screen: chip Language bấm → popup en/vi/zh (`setLang` lưu+reload), chip Currency bấm → popup USD/EUR/CNY/VND. ⚠️ Circle modal luôn English + chuỗi mới hardcode English → VI/ZH partial. **(3) Tiền hiển thị thêm CNY/VND:** `data.js` SUPPORTED_CURRENCIES + CURRENCY_LABEL (¥/₫) + displayNum (VND 0 lẻ); `chain.js` `fetchFiatRates()` lấy giá USDC theo cny/vnd rồi nghịch đảo (USD/đơn vị), fallback CNY 0.14 / VND 0.0000395; cập nhật fallback rate ở BalanceHeader/HomeSend/TxHistory/SendConfirm. BalanceHeader thêm `amountFontSize` auto-co (số VND rất dài). **(4) Security:** bỏ hàng "Recovery method / Coming soon" (thừa) → còn Email/Ví/Đổi PIN (h2-4).
