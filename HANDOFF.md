...
- **Caret ô nhập số khi TRỐNG = CHỈ dấu `_` xám nhấp nháy** (user chốt 07-20b tại Swap): KHÔNG vẽ số 0 mờ đứng cạnh caret ("0 là 0, _ là _"). **Màn Swap — QUY HOẠCH VÙNG user chốt 07-20 (bản cuối, thay 2 bản nháp cùng ngày):** nội dung swap = hàng 2 → NỬA hàng 6, bắt đầu từ ĐẦU hàng 2 (không dồn giữa): **You pay = 2 HÀNG · You receive = 2 HÀNG** (nút ⇅ đè ranh giới, margin -18/-18 = khe 8px) · **Fee/Rate = nửa hàng 6**, 1 dòng chữ NHỎ fs-item 17 (= font nút "Hold to show tokens"), KHÔNG box, tách 2 ĐẦU (07-20b: bản gộp giữa "mờ nhạt"): `Rate: 1 EURC ~ 1.1400 USDC` căn TRÁI – `Fee: <$0.01` căn PHẢI, nhãn xám + SỐ LIỆU ĐEN medium; nửa sau hàng 6 để trống. Card 3 hàng nội dung: nhãn · [chip token ▼ trái | SỐ TO phải, KHÔNG lặp tên token sau số] · [Available trái | ~$quy đổi phải]; chữ phụ `--fs-body` 19, chip logo 32 + chữ 19, số to base 52 co giãn (`amountFontSize(str, 52, 8)`). **Numpad Swap:** bấm SỐ TIỀN You pay → sheet trượt từ dưới lên chiếm **nửa hàng 6 → hàng 10** (55→100dvh): NỀN XÁM surface + PHÍM TRẮNG tile bo 12 khe 8px (nổi button), KHÔNG khoảng trắng thừa trên đầu, overlay TRONG SUỐT (user chốt: numpad bật mà màn chính mờ đi là SAI); trong sheet Numpad 30dvh + nút Back/Done pill 44% ở 85-95dvh (khớp .row10-dual) + đệm 5dvh. Gõ live cập nhật số + pct + estimate; Back = hủy số vừa gõ, Done/bấm ra ngoài = giữ; slider + chip gợi ý giữ nguyên.
...
- **Slider % (PctSlider):** nam châm hút mốc `SNAP_ZONE = ±9%` (07-20d, trước ±2% "bấm chính xác lắm mới trúng"); mốc dot 14px, nhãn % fs-item 17 + BẤM ĐƯỢC (chạm nhãn nhảy tới mốc). Marker 0/25/50/75/100.
- **ShowQR (xem/ tạo QR nhận tiền):** QR TO = `min(30dvh,78vw)` (bằng QR màn Nhận), cao 3 hàng (2-4); hàng 5→8 số tiền TO `fs-amount` light + phụ đề `fs-md-lg`; nút Share/Back ở `.row10-dual` (9-10). Tiêu đề ĐỘNG: có tên → `QR Storage – <tên>`, không tên → `Create receive QR`. **Add-to-library (SavedQRList popup):** ô Amount label kèm ký hiệu tiền tệ mặc định — `Amount (${displaySymbol(getDisplayCurrency())})` (USDC→$, EURC→€).
- **Send hints rõ nghĩa hơn** (07-20d, bắt chước Receive): Contacts=Send to a saved contact · Scan QR=Scan a QR to pay · Paste=Paste a wallet address. Box token màn Send VẪN scroll khi nhiều token (đừng bỏ overflow).
