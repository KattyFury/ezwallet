...
- **HINT (NotifArea) = `Label: mô tả`, label GẠCH CHÂN + BẤM ĐƯỢC** (user chốt 07-21): mỗi dòng `{label, desc, onClick}` → label gạch chân (`textUnderlineOffset 3`, **KHÔNG bold**, giữ dấu `:` sau label) bấm vào đi ĐÚNG nơi nút cùng tên ở hàng 9 dẫn tới; câu dài ĐƯỢC XUỐNG DÒNG (bỏ nowrap/ellipsis — chỉ thông báo THẬT mới giữ 1 dòng cắt "…").
  **Text chốt (đừng tự sửa):** Send — `Contacts: Save wallet addresses of people you know` → Contacts · `Scan QR: Scan the receiver's QR code to send` → QRScanner · `Paste Address: Paste the receiver's wallet address to send` → PasteAddress. Receive — `QR Storage: Keep your most frequently used QR codes` → SavedQRList · `Create QR: Create a QR code to receive the exact amount` → CreateQR · `Share Address: Share your wallet address in many ways` → handleShare (⚠️ "in many ways", KHÔNG phải "via many ways" — via đã nghĩa là "qua/bằng", ghép vào là sai ngữ pháp).
...
- **SendAmount — NOTE MẶC ĐỊNH** (user chốt 07-20e): cụm Send-to/số tiền/note gom 1 flex column `gridRow 2/6` gap 2dvh (xích gần, hết rời rạc). Ô note có **icon `option` bên PHẢI** (nút 52×52 nền surface) → popup "Set your default note" (input "Type here", Back/Save), lưu `localStorage ez_default_note`. memo khởi tạo = note mặc định → hiện như VALUE thật (không phải placeholder mờ); **click vào ô note lần đầu (onFocus) mà đang là note default → XOÁ để gõ mới** (`noteTouched` chặn xoá lại). Mọi lần gửi memo tự mang note mặc định.
