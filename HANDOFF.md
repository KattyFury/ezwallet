...
> **Session 11 (2026-07-03) — SWAP EXECUTE build lại trên Multicall3From (`583f32c`) + chừa 1 USDC phí:**
> - **Kiến trúc mới (đúng tech Circle + Arc):** Stablecoin Kit `/swap` → `instructions[]` → gộp TẤT CẢ vào MỘT `Multicall3From.aggregate3(...)` (`0x522f...47D0`, allowFailure=false) → MỘT contractExecution challenge → user ký 1 lần PIN. Root cause fail cũ = 2 challenge riêng, swap simulate trước khi approve mine → revert; batch atomic xử tận gốc. CallFrom giữ msg.sender — cùng cơ chế Memo contract đã verify on-chain.
> - **Encoder `aggregate3((address,bool,bytes)[])` tự viết** (không kéo viem vào CF Function) — **VERIFIED khớp byte với viem** (3 case). Selector `0x82ad56cb`. Nằm ở `functions/api/swap.js` + bản sao `dev-server.js`.
> - **Shape instructions VERIFIED bằng gọi Kit thật:** EURC→USDC amount 5 → 2 instructions `{target, data, value:0}` (approve-kiểu-Kit + swap; target KHÔNG phải token contract — contract riêng của Kit, kệ nó, batch nguyên xi theo thứ tự). ⚠️ Amount nhỏ bị 422 `code 331001` "insufficient after fees" (partnerFee ~1) — swap quá nhỏ Kit từ chối, UI sẽ hiện lỗi này nguyên văn.
> - **Chừa phí mạng:** `data.js` `GAS_RESERVE_USDC = 1` + `spendableOf(sym, bal)` (chỉ USDC bị trừ 1) — áp vào SendAmount (khả dụng) + Swap (Available/Max/validation, hiện note "1 USDC is kept for network fees" khi from=USDC). Lý do user: khách bấm gửi/swap HẾT thì không còn USDC trả gas, kẹt ví.
> - **Swap.jsx làm lại theo design system:** logo token thật `/tokens/*.png`, `.num`/fs/fw token, popup chọn token kiểu popup tiền tệ SendAmount (neo nửa trên), chip Max viền xanh, numpad rows 7-9 + nút, NavBar tab "Đổi tiền" BẬT lại. Mặc định from=EURC→to=USDC (đúng kịch bản cứu "hết USDC"). `refreshSession()` TRƯỚC khi tạo challenge (token 60'). Chuỗi mới hardcode English.
> - **CHƯA test on-chain thật** (cần PIN user trên deploy). Cần user test: swap EURC→USDC lượng ≥ vài đơn vị (nhỏ quá dính 331001), nhập PIN 1 lần, xong check số dư + ArcScan. Nếu revert on-chain → đọc tx trên ArcScan xem subcall nào fail.
