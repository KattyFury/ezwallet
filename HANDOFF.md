...
- **🔴🔴🔴 08-04d/f/g – ĐÃ TẮT CẢ `setCustomSecurityQuestions()` LẪN `Localizations.securityQuestions` — màn Câu hỏi bảo mật VẪN RỖNG qua 3 LẦN ĐO LIÊN TIẾP với 3 thay đổi khác nhau:**
  1. `setCustomSecurityQuestions({ questions: [8 câu VN], securityConfirmItems })` → rỗng.
  2. Bỏ `questions`, chỉ còn `{ securityConfirmItems }` → **rỗng y hệt**.
  3. Tắt hẳn `setCustomSecurityQuestions()` (không gọi nữa) → **rỗng y hệt lần nữa**.
  Biến chung DUY NHẤT còn lại giữa cả 3 lần: khối `Localizations.securityQuestions` (title/questionHeader/questionPlaceholder/requiredMark/answerHeader/answerPlaceholder/answerHintHeader/answerHintPlaceholder) — title CÓ render đúng "Câu hỏi bảo mật" (khối này được đọc, không bị từ chối toàn bộ) nhưng phần form bên dưới rỗng. **Đã comment out toàn bộ khối này** (`circleLocalizations.js`) — nếu màn VẪN rỗng sau lần deploy này thì lỗi không nằm ở phía mình (code đã tắt sạch mọi thứ liên quan tới câu hỏi bảo mật), cần liên hệ Circle support kèm ảnh chụp. **ĐỪNG bật lại bất kỳ field nào trong `securityQuestions`/gọi lại `setCustomSecurityQuestions()`** cho tới khi xác nhận được nguyên nhân thật (lý tưởng nhất: xin được lỗi console thật từ trình duyệt, xem mục 9).
