...
- Box xám đang phủ: 2 card màn Swap (Fee/Rate từ 07-20 là dòng chữ trần, KHÔNG box) · vùng token HomeSend (hàng 3→5.5, `height calc(100%+5dvh)`) · list Contacts/TxHistory (row 2-8) · **SavedQRList (row 2-8, 07-23): lưới 2 CỘT (3 cột cũ QR bé), mỗi QR = box TRẮNG viền xám 1.5 bo 16 + drop shadow .25 + X xóa góc trên-phải; box xám padding 10 + gap 10 (box trắng cách lề xám đúng 10px), QR CO GIÃN theo box (svg width 100% height AUTO — viewBox tự giữ vuông; ép height 100%/aspectRatio từng lệch 3px, khung cứng 104 từng méo) + tên fs-item 17 + tiền fs-label 15. ⚠️ 2 BÀI HỌC: (1) KHÔNG dùng .scroll-thin TRONG box xám (margin-right -20 làm tràn phải — PC có scrollbar-gutter bù nên nhìn ổn, iOS KHÔNG → vỡ; dùng .scroll-hidden); (2) verify layout phải đo THÊM 375px, đừng chỉ 390.** · Language (2-3) · Security (2-4) · About (2-8) · mọi ô nhập (`.address-input`, `.memo-row`; lỗi = inset shadow đỏ).
