...
- **Numpad = hàng 6.5–8.5** (`gridRow 6/9`, spacer 0.5 + Numpad 2.5), nút Back/Continue riêng ở `.row10-dual` (SendAmount, CreateQR). **Card Swap TỐI GIẢN (07-20, user chốt "tinh giản để chữ to hơn cho người già"):** mỗi card 3 hàng — nhãn · [chip token ▼ trái | SỐ TO phải, KHÔNG lặp tên token sau số] · [Available trái | ~$quy đổi phải]; Fee+Rate GỘP 1 dòng `Fee <$0.01 | Rate ...` trong card mỏng. Chữ phụ = `--fs-item` 17; số to = 44px co giãn theo độ dài (`amountFontSize(str, 44, 8)`), `--fs-huge` 38 không còn dùng ở Swap. **Riêng numpad Swap (07-20):** bấm SỐ TIỀN card "You pay" → bottom-sheet `.sheet-overlay/.sheet` trượt từ dưới lên (như PIN); layout TRONG sheet Y CHANG SendAmount (user chốt "đồng bộ giao diện"): sheet = nửa dưới màn 50dvh, spacer 0.75 + Numpad 2.5 + đệm 0.75 + hàng nút Back/Done pill 44% (chuẩn .row10-dual, không thêm gì khác). Gõ live cập nhật số + pct + estimate; Back = hủy số vừa gõ (khôi phục như trước khi mở), Done/nền tối = giữ; slider + chip gợi ý giữ nguyên.
