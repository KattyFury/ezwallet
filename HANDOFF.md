...
**Cập nhật:** 2026-07-06 (session 23 — MỞ LẠI chọn ngôn ngữ (en/vi/zh) + tiền tệ hiển thị (USD/EUR/CNY/VND); TxHistory "Add to Contacts" + mờ đáy list; Security bỏ Recovery. [S22: local dev-server, lề 20px, scrollbar gutter, nút vị trí 5/9, no line xám...])
...
- **Ngôn ngữ user CHỌN ĐƯỢC (S23, mở lại):** `i18n.js` `LANG = localStorage.ez_lang || 'en'` (mặc định English). Popup Language & Currency chọn en/vi/zh → `setLang()` lưu + reload. ⚠️ Modal PIN/xác nhận của Circle LUÔN tiếng Anh (không đổi được); nhiều chuỗi MỚI hardcode English (không qua `t()`) → chọn VI/ZH còn LẪN Anh vài chỗ — cần dịch bổ sung nếu muốn full. `detect()` (browser) chỉ dùng khi chưa từng chọn.
