...
> - **CẦN USER TEST trên deploy:** login Google LẠI (để có refreshToken) → Đổi PIN. Nếu vẫn fail, lỗi giờ tự nói rõ (status + code) — gửi screenshot là đủ chẩn đoán. Nếu PUT vẫn 403 cho SSO → Circle chặn đổi PIN cho user SSO ở tầng platform → phương án: ẩn nút Đổi PIN cho user Google (kèm giải thích), hoặc user đã nói sẽ yêu cầu ẩn social login.
