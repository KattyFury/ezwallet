...
- **SERVICE HUB (08-12)** – `src/screens/ServiceHub.jsx`, **tab 1 của NavBar** (icon `hub`, nhãn "Services"), thay chỗ tab Swap cũ. Lưới 2 cột ô VUÔNG nổi trong box xám, hình học copy màn Kho QR. Danh sách dịch vụ = mảng `SERVICES` trong file đó, thêm dịch vụ = thêm 1 dòng:
  ...
  - ⚠️ **Màn Swap không còn tab riêng** ⇒ hàng 10 của nó là **nút Exit ĐỎ** (`.btn-error`) về ServiceHub, KHÔNG phải NavBar. **ĐỪNG dùng class `.row10-single` cho nút này**: class đó `position:absolute; top:85dvh` = neo tâm **90dvh (vị trí hàng 9)**, mà hàng 9 màn Swap đang có nút "Swap" (đáy 87dvh) → 2 nút dính sát nhau. Phải để `gridRow 10` (tâm 95dvh = đúng băng NavBar vừa nhả). Đo Playwright 3 cỡ màn: khe 22–47px, không tràn.
