...
- **HINT (NotifArea) = `Label: mô tả`, chữ PHẲNG (KHÔNG bold, KHÔNG gạch chân) nhưng VẪN BẤM ĐƯỢC** (user chốt 07-21 bản cuối — đã thử bold rồi gạch chân, đều bỏ): mỗi dòng `{label, desc, onClick}`, bấm vào label đi ĐÚNG nơi nút cùng tên ở hàng 9 dẫn tới; câu dài ĐƯỢC XUỐNG DÒNG (bỏ nowrap/ellipsis — chỉ thông báo THẬT mới giữ 1 dòng cắt "…"). **Nhãn hint PHẢI TRÙNG nhãn nút hàng 9** (đừng đặt "Paste Address"/"Share Address" lệch với nút "Paste"/"Share").
  **Text chốt (đừng tự sửa):** Send — `Contacts: Save wallet addresses of people you know` → Contacts · `Scan QR: Scan the receiver's QR code to send` → QRScanner · `Paste: Paste the receiver's wallet address to send` → PasteAddress. Receive — `QR Storage: Keep your most frequently used QR codes` → SavedQRList · `Create QR: Create a QR code to receive the exact amount` → CreateQR · `Share: Share your wallet address in many ways` → handleShare (⚠️ "in many ways", KHÔNG phải "via many ways" — via đã nghĩa là "qua/bằng", ghép vào là sai ngữ pháp).
