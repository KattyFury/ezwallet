...
- **HINT (NotifArea) = CÂU ĐỦ NGHĨA, từ khoá GẠCH CHÂN + BẤM ĐƯỢC** (user chốt 07-21, thay format `Label: desc` bold cũ): mỗi dòng `{label, desc, onClick}` → label gạch chân (`textUnderlineOffset 3`, **KHÔNG bold**) bấm vào đi ĐÚNG nơi nút cùng tên ở hàng 9 dẫn tới; câu dài ĐƯỢC XUỐNG DÒNG (bỏ nowrap/ellipsis — chỉ thông báo THẬT mới giữ 1 dòng cắt "…"). Send: Contacts→Contacts · Scan QR→QRScanner · Paste→PasteAddress. Receive: QR Storage→SavedQRList · Create QR→CreateQR · Share→handleShare. ⚠️ Hint dài hơn = khi có thông báo thật, hint bị đẩy lên + mờ ở mép trên nhiều hơn (đúng thiết kế ưu tiên: hint thấp nhất). Box token màn Send VẪN scroll khi nhiều token (đừng bỏ overflow).
