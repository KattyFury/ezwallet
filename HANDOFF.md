...
- Box xám đang phủ: 2 card màn Swap (Fee/Rate từ 07-20 là dòng chữ trần, KHÔNG box) · vùng token HomeSend (hàng 3→5.5, `height calc(100%+5dvh)`) · list Contacts/TxHistory (row 2-8) · **SavedQRList (row 2-8, 07-23): lưới 2 CỘT (3 cột cũ QR bé), mỗi QR = box TRẮNG viền xám 1.5 bo 16 + drop shadow .25, QR 104 + tên fs-item 17 + tiền fs-label 15, X xóa góc trên-phải** · Language (2-3) · Security (2-4) · About (2-8) · mọi ô nhập (`.address-input`, `.memo-row`; lỗi = inset shadow đỏ).
