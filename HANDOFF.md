...
- **⚠️ CHƯA XÁC NHẬN được (cần test live) – màn "Xác nhận bảo mật" bắt gõ đúng 1 cụm từ để mở khoá nút Tiếp tục** (không phải nhập lại câu hỏi/trả lời đã đặt – 2 luồng khác nhau). Bản gốc English bắt gõ "I agree" (đo thật trước khi sửa). Field `inputMatch` trong `Localizations.securityConfirm` được set thử thành `'Tôi đồng ý'` (đồng bộ với `headline`/`inputPlaceholder` cũng đổi theo) nhưng **KHÔNG có tài liệu nào xác nhận field này thật sự đổi được cụm từ SDK validate, hay chỉ là chữ hiển thị suông** (SDK là iframe cross-origin `pw-auth.circle.com`, không đọc được source để kiểm chứng). Test trên deploy: gõ "Tôi đồng ý" → nút Tiếp tục có sáng lên không? KHÔNG sáng = field này chỉ cosmetic, phải đổi `headline`/`inputPlaceholder` lại thành hướng dẫn gõ "I agree" (tiếng Anh, đã biết chắc hoạt động).
- **✅ 08-04c: đã thay bộ câu hỏi bảo mật mặc định (tiếng Anh, kiểu "What is your father's middle name?") bằng 8 câu tiếng Việt quen thuộc** (`CIRCLE_SECURITY_QUESTIONS` trong `circleLocalizations.js`, truyền qua `setCustomSecurityQuestions({ questions })` — cùng lệnh với `securityConfirmItems`). Chọn câu KHÔNG đổi theo thời gian (né "món ăn/màu yêu thích hiện tại" — dễ đổi ý, khó nhớ lại đúng). `requiredCount` giữ mặc định 2. Test trên deploy: mở dropdown "Câu hỏi" ở màn Câu hỏi bảo mật phải thấy đủ 8 câu tiếng Việt.
